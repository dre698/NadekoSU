...
NativeBridge(getAppProfile, jobject, jstring pkg, jint uid) {
	...
	jclass cls = GetEnvironment()->FindClass(env, "com/nadekosu/Natives$Profile");
...
NativeBridgeNP(getKernelPatchImplementation, jobject) {
	...
	jclass cls = GetEnvironment()->FindClass(env,
                                             "com/nadekosu/Natives$KernelPatchImplementation");
	if (cls == nullptr) {
	...
	jmethodID valuesMethod = GetEnvironment()->GetStaticMethodID(env, cls, "values",
                                                                 "()[Lcom/nadekosu/Natives$KernelPatchImplementation;");
...
NativeBridgeNP(getDynamicManager, jobject) {
	...
	jobject obj = CREATE_JAVA_OBJECT("com/nadekosu/Natives$DynamicManagerConfig");
	jclass cls = GetEnvironment()->FindClass(env, "com/nadekosu/Natives$DynamicManagerConfig");
...
NativeBridgeNP(getManagersList, jobject) {
    ...
    jobject obj = CREATE_JAVA_OBJECT("com/nadekosu/Natives$ManagersList");
    jclass managerListCls = GetEnvironment()->FindClass(env,
                                                        "com/nadekosu/Natives$ManagersList");
    ...
    if (cmd && count > 0) {
        for (int i = 0; i < count; i++) {
            jobject managerInfo = CREATE_JAVA_OBJECT_WITH_PARAMS(
                    "com/nadekosu/Natives$ManagerInfo",
                    "(II)V",
...
Java_com_nadekosu_nadekosu_magica_AppZygotePreload_forkDontCareAndExecKsud(JNIEnv *env,
                                                                           jclass clazz,
